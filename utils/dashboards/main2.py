...
@app.callback(
    dash.dependencies.Output("worktime-graph", "figure"), [dash.dependencies.Input("type-of-work-dropdown", "value")]
)
def update_figure(selected_type_of_work):
    if not selected_type_of_work:
        filtered_df = df
    else:
        filtered_df = df[df["type_of_work"].isin(selected_type_of_work)]
    data = []
